...
(function () {
    const sendPacket = async (event) => {
        ...
        if (message) {
            console.log(message);
            eventSys.emit("sendPacket", JSON.stringify({ type: "message", data: { message } }));
        }
    };
    ...
    const sendName = async (event) => {
        ...
        if (name) {
            console.log(name);
            eventSys.emit("sendPacket", JSON.stringify({ type: "user_name", data: { name } }));
        }
    };
    ...
    document
        .querySelector('#send-name')
        .addEventListener('click', sendName);
})();
